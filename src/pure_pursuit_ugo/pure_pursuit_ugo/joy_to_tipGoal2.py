...
class joy_to_tipGoal_node2(Node):
    def __init__(self):
        super().__init__('joy_to_tipGoal_node2')
        ...
        self.goal_position.x = 6.0
        self.goal_position.y = 0.0
        self.goal_position.z = 0.0
        self.bucket_end_angle = Float64()
        self.bucket_end_angle.data = mt.radians(180)
        
        self.last_joy = Joy()
        self.get_logger().info("joy_to_tipGoal_node2 Started (with angle holding)")
    ...
    def publish_commands(self):
        ...
        # 刃先の目標位置を制御
        self.goal_position.x += msg.axes[4] * gain * dt  # 前後
        
        self.goal_position.y += msg.axes[3] * gain * dt  # 左右

        self.goal_position.z += msg.buttons[5] * gain * dt  # 上
        self.goal_position.z -= msg.buttons[4] * gain * dt  # 下

        # 右スティックの左右（x軸）でバケットの水平方向の角度を制御
        self.bucket_end_angle.data += msg.buttons[7] * gain * dt # 増
        self.bucket_end_angle.data -= msg.buttons[6] * gain * dt # 減
        ...
        # ログ出力
        self.get_logger().info(f'Goal position updated: x={self.goal_position.x:.1f}, y={self.goal_position.y:.1f}, z={self.goal_position.z:.1f}')
        self.get_logger().info(f'Bucket end angle updated: {mt.degrees(self.bucket_end_angle.data):.1f}')
        
def main(args=None):
    rclpy.init(args=args)
    node = joy_to_tipGoal_node2()
    rclpy.spin(node)
    rclpy.shutdown()
